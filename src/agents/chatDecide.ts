...
export async function decideNextStep(params: {
  ...
  const ebookNote =
    agent === 'brain'
      ? ''
      : `\n\n[전자책 노하우는 이미 내장됨 — 중요]
대표님 스레드 마케팅 전자책("광고비 0원으로 고객이 먼저 연락오는 스레드 마케팅")의 핵심 노하우(훅·스토리 구조·
고객 중심·CTA·판매 심리 등)는 네가 실제로 기획/작성을 실행할 때 시스템에 "이미" 주입된다. 따라서 "전자책 내용을
알 수 없다"거나 "전자책 핵심을 텍스트로 붙여달라"고 절대 되묻지 마라 — 이미 알고 있는 것으로 간주하고 바로 작업(act)한다.
