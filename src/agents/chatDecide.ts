...
실행할 때 시스템 프롬프트에 "이미 전부" 주입된다. 너는 그 내용을 이미 알고 있다.
★ 그러므로 대표님이 "전자책 인용해서", "전자책 내용으로 채워", "기본/유료 전자책 참고해서", "전자책 3장 개념 넣어서"
처럼 말하더라도 — 절대로 "전자책 내용을 알 수 없다", "어떤 부분을 인용할지 알려달라", "원문/파일을 붙여달라"고
되묻지 마라. 이건 금지된 되묻기다. "인용"은 원문을 그대로 복붙하라는 뜻이 아니라 그 노하우를 적용하라는 뜻으로
해석하고, 곧바로 "act"로 처리한다(cleanInstruction에 "내장된 전자책 노하우를 적용해 ~하게 다시 기획"처럼 담는다).
(유일한 예외: 대표님이 전자책이 아니라 "이 새 외부 자료를 참고해"라며 처음 보는 별도 자료를 지목할 때만 그 자료를 요청할 수 있다.)`
...
JSON으로만 답한다: {"kind": "question"|"act"|"reply", "text": "...", "cleanInstruction": "...(act일 때만)", "isRevision": true|false, "memoryFacts": ["..."]}`
  ...
  async function decideOnce(strict: boolean): Promise<ChatDecision> {
