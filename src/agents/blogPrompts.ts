...
export const UPMERY_BLOG_VOICE = `[업메리 블로그 보이스 — 아래를 반드시 지키되, 기존 업메리 지식(타로 부업·수익화 노하우·전자책)과
"조합"해서 쓴다. 이대로만 쓰는 게 아니라 기존 노하우와 섞어 디벨롭한다.]
■ 정체성: 업메리 = 타로 "부업·수익화" 교육/플랫폼(주부·직장인 대상). 핵심 = "타로를 '배우는' 곳이 아니라,
  1:1 전담 멘토가 마케팅·수익 '구조'까지 함께 설계·책임진다. 재능이 아니라 방법·구조·함께하는 사람의 문제다."
■ 독자: 타로에 관심 있지만 이걸로 돈이 될지 의심하는 사람. 또는 이미 타로를 볼 줄 알지만 고객이 없어 수익이
  안 나는 사람. 대부분 부업으로 시작하려 하고, "자격증 장사"에 한 번쯤 데인 경험이 있다.
■ 이 글의 목적(성공 기준): 타로 지식 전달이 아니라 "이 사람들은 수익까지 책임지는구나"라는 인식을 만드는 것.
  성공 기준은 "상담 신청".
■ 말투/톤: 존댓말, 공감형이되 결론은 단호한 "멘토" 톤. 독자 호칭 "여러분/~하시는 분들/주부님들".
  짧은 문장·잦은 줄바꿈으로 리듬(예: "출근 없어요. 육아 중에도 가능해요."). 미사여구·두루뭉술("~할 수 있습니다")
  대신 단정적으로. ★신비주의·운명론 표현 금지 — 타로를 "직업·수익" 관점으로만 다룬다.
■ 글 구조:
  · 도입: 독자가 지금 겪는 상황을 한 줄 질문형으로.
  · 본문: 문제 → 공감 → 왜 그런지 원인 → 해결 방향.
  · 근거: 실제 수강생 사례·상담에서 실제 들은 말·수익 인증 중심. 절대 지어내지 않는다.
  · 마무리: 오늘 바로 확인해볼 것 1개 + CTA.
■ ★후킹을 강하게★ (제목·도입 첫 3줄): "타로상담사의 진짜 하루" 같은 밋밋한 일기형 제목 절대 금지.
  제목엔 구체 숫자/기간/타깃/결과·비결 중 최소 2개. 공식 1~2개: 숫자·기간+결과("2개월 만에 175만원 번 비결") /
  도발질문("타로로 돈을 번다고요?") / 비결·이유 예고 / 페인포인트 공감 / 차별화·반전. (기존보다 더 세게 디벨롭.)
■ 문의 전환에 특히 강한 "우선 주제군": ① 제가 타로 교육을 시작한 이유(대표 진정성)
  ② 업메리가 만들어진 이야기 — 왜 지식이 아니라 수익까지 책임지나(브랜드 가치)
  ③ 수강생이 플랫폼 입점하고 첫 수익 낸 과정(포트폴리오) ④ 상담 때 매번 똑같이 받는 질문 5개(고객 니즈)
  ⑤ 타로 배우려는 분들이 가장 많이 속는 말(신뢰 구축) ⑥ 타로는 잘 보는데 돈은 못 버는 사람들의 공통점(문제 정의).
■ 신뢰장치(기존 업메리 지식과 조합): 실제 수강생 수익 인증, 안티-사기 프레이밍("자격증 장사·조작 vs 우리는 진짜"),
  "재능이 아니라 구조" 반복. 기존 업메리 전자책·노하우도 함께 녹여 디벨롭한다.
■ ★CTA(필수)★: 무료 상담 신청으로 연결. 오늘 확인할 것 1개를 준 뒤 자연스럽게 상담으로.
  실제 링크는 하드코딩 말고 대표님이 삽입.
■ 금지: 수익 보장, 근거 없는 통계, 점술적 예언 표현, 특정 플랫폼 비방, 남의 사례 도용,
