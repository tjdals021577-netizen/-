...
## 레이더 (GA4 자동 수집) — 2026-07-13부터
- 로직: `api/_lib/ga4.ts`(서비스 계정 JWT로 GA4 Data API 직접 호출, 별도 SDK 없음),
  `api/cron/radar.ts`(매일 07:50 KST — 모닝 크론 10분 전에 실행)
- 브랜드별 GA4 속성을 각각 연결한다 — 서비스 계정(`GA4_SERVICE_ACCOUNT_KEY`)은 공용,
  속성 ID만 브랜드별 환경변수(`GA4_PROPERTY_ID_UPMERY`, `GA4_PROPERTY_ID_MAJALNAM`)로 분리.
  아직 연결 안 한 브랜드는 해당 환경변수가 비어있으면 조용히 건너뜀(에러 아님) —
  하나씩 순서대로 연결해가는 걸 전제로 설계함
- 결과는 Supabase `radar_snapshots` 테이블에 브랜드별로 하루 한 건씩 쌓임(방문자 수·세션·
  전환·인기 페이지 top5·유입경로 top5)
- 이 데이터를 **모닝 크론이 자동으로 읽어서** 브리핑에 반영(`api/cron/morning.ts`의
  `buildRadarText`) — "어제 방문자 반응이 어땠는지 보고 다음 방향 추천"까지 매일 아침
  자동으로 나옴(원래 목표였던 기능)
- **자격증명 설정 방법**은 `deployment.md`의 "레이더(GA4) 연결" 절 참고 — 구글 클라우드
  콘솔에서 서비스 계정 만들고 GA4 속성에 뷰어로 추가하는 절차, 새 브랜드 추가할 때마다
  속성 ID 환경변수만 하나 더 추가하면 됨(서비스 계정은 재사용)
- 유튜브/메타/아임웹은 아직 미연결 — 각각 OAuth 앱 등록 등 별도 절차 필요, 순차 진행 예정
...
- **브랜드 토글(헤더)과 무관하게 업메리/마잘남 두 섹션을 항상 동시에 보여줌** — 대시보드만
  ...
  다르게 동작함
- 방문자/유입경로/인기 페이지/전환은 `src/lib/radarStore.ts`로 Supabase `radar_snapshots`의
  최신 행을 브랜드별로 읽어서 표시(레이더 크론이 연결 안 된 브랜드는 여전히 `—`로 보임).
  결제 전환은 GA4 쪽에 전환 이벤트가 등록돼 있어야 값이 잡힘(아직 미설정이면 0으로 표시됨)
