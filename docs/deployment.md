...
단계는 자동 감지가 안 되고, 대표님이 채널 탭에서 직접 체크하는 수동 항목으로 남겨뒀다
(억지로 가짜 자동화를 붙이지 않기로 결정).

블로그용 사진은 "블로그" 탭의 "사진 업로드" 섹션(`ChannelWorkspaceScreen.tsx`, 2026-07-14
구조 개편 전엔 독립된 `사진함` 탭)에서 날짜·브랜드별로 미리 올려두면 `content_photos`
테이블에 저장되고, 그 날짜의 content-schedule 크론이 실행될 때 자동으로 반영된다. 사진이
없으면 텍스트만으로(실제 상위노출 글 구조 웹서치 참고) 생성된다.
